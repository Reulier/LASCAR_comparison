...
public class RasterLayer {
	...
	public RasterLayer(String name, File rasterfile, boolean forceLonLatAxisOrder){
		...
		// Wrap the read coverage in a writable object so that this layer can be written to.
	...
	public float getFloatGridValue(int x, int y) {
		return gridCoverage.evaluate(new GridCoordinates2D(x, y), 
				new float[getNumBands()])[0];
	}
	
	public double getDoubleGridValue(int x, int y) {
		return gridCoverage.evaluate(new GridCoordinates2D(x, y), 
				new double[getNumBands()])[0];
	}
	...
	public int getWidth() {
		return gridCoverage.getGridGeometry().getGridRange().getHigh().getCoordinateValues()[0] + 1;
	}

	public int getHeight() {
		return gridCoverage.getGridGeometry().getGridRange().getHigh().getCoordinateValues()[1] + 1;
	}

	public Envelope getEnvelope() {
		System.out.println(gridCoverage.getProperties().keySet());
